...
static int vnode_cmp(const void *a, const void *b) {
    return (((VNode*)a)->hash > ((VNode*)b)->hash) -
           (((VNode*)a)->hash < ((VNode*)b)->hash);
}
...
static void build_ring(Cluster *c) {
    ...
    for (int i = 0; i < c->count; i++) {
        if (c->nodes[i].status == NODE_DEAD) continue;
...
int cluster_load_config(Cluster *c, const char *path, int self_id) {
    ...
    FILE *f = fopen(path, "r");
    if (!f) { perror("cluster config"); return 0; }

    char line[256];
    while (fgets(line, sizeof(line), f)) {
        if (line[0] == '#' || line[0] == '\n') continue;
        Node *n = &c->nodes[c->count];
        if (sscanf(line, "%d %63s %d", &n->id, n->host, &n->port) != 3) continue;
        n->status = NODE_ALIVE;
        n->fd     = -1;
        c->count++;
        if (c->count >= MAX_NODES) break;
    }
    fclose(f);
...
int cluster_hash_key(const Cluster *c, const char *key) {
    ...
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        if (ring[mid].hash >= h) { result = mid; hi = mid - 1; }
        else lo = mid + 1;
    }
...
void cluster_mark_dead(Cluster *c, int id) {
    ...
    if (n) { n->status = NODE_DEAD; build_ring(c); }
}

void cluster_mark_alive(Cluster *c, int id) {
    Node *n = cluster_get_node(c, id);
    if (n) { n->status = NODE_ALIVE; build_ring(c); }
}

int cluster_quorum(const Cluster *c) {
    int alive = 0;
    for (int i = 0; i < c->count; i++)
        if (c->nodes[i].status == NODE_ALIVE) alive++;
    return (alive / 2) + 1;
}
