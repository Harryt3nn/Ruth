...
int protocol_parse_command(const char *line, Command *out) {
    ...
    // HELLO <pubkey_hex>
    if (sscanf(line, "HELLO %64s", out->pubkey_hex) == 1
...
int protocol_parse_response(const char *raw, Response *out) {
    ...
    if (strncmp(raw, "OK",    2) == 0) { out->type = RESP_OK;   return 1; }
    if (strncmp(raw, "NIL",   3) == 0) { out->type = RESP_NIL;  return 1; }
    if (strncmp(raw, "PONG",  4) == 0) { out->type = RESP_PONG; return 1; }
