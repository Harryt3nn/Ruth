...
{
    if (write(conn->fd, cmd, strlen(cmd)) < 0) 
    {
        return 0;
    }
...
static int do_handshake(DbConn *conn) 
{
    ...
    return crypto_derive_session(conn->session.keypair.priv, server_pub, &conn->session.key);
}
...
{
    if (!session_needs_rotation(&conn->session)) 
    {
        return 1;
    }
...
static int encrypt_to_hex(DbConn *conn, const char *value, char *out_hex, size_t hex_size) 
{
    uint8_t cipher[2048];
    size_t  cipher_len;
    if (!crypto_encrypt(&conn->session.key, (uint8_t*)value, strlen(value), cipher, &cipher_len))
    {
        return 0;
    } 
    if (cipher_len * 2 + 1 > hex_size)
    {
        return 0;
    } 
...
static char *decrypt_from_hex(DbConn *conn, const char *hex) 
{
    ...
    if (!crypto_from_hex(hex, raw, hex_len / 2)) 
    { 
    ...
    size_t  plain_len;
    if (!crypto_decrypt(&conn->session.key, raw, hex_len / 2, plaintext, &plain_len)) 
    {
        free(raw);
        return NULL;
    }
