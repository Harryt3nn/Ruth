...
static void start_election(RaftState *r) {
    ...
    for (int i = 0; i < r->cluster->count; i++) {
        ...
        if (n->id == r->self_id || n->status == NODE_DEAD) continue;
...
static void send_heartbeats(RaftState *r) {
    ...
    for (int i = 0; i < r->peer_count; i++) {
        ...
        if (!n || n->status == NODE_DEAD) continue;
        send_msg(n->fd, &msg);
    }
}
...
static void handle_vote_resp(RaftState *r, const RaftMsg *resp) {
    ...
    if (resp->term > r->current_term) { become_follower(r, resp->term); return; }
...
static void handle_append_req(RaftState *r, const RaftMsg *req, int reply_fd) {
    ...
    for (int i = 0; i < req->entry_count; i++) {
        ...
        if (r->apply_fn) r->apply_fn(&e, r->apply_userdata);
    }

    // Update commit index
    if (req->leader_commit > r->commit_index) {
        r->commit_index = req->leader_commit < r->wal.last_index
            ? req->leader_commit : r->wal.last_index;
        apply_committed(r);
    }
...
static void handle_append_resp(RaftState *r, const RaftMsg *resp) {
    ...
    PeerState *peer = NULL;
    for (int i = 0; i < r->peer_count; i++)
        if (r->peers[i].node_id == resp->from_id) { peer = &r->peers[i]; break; }
    if (!peer) return;
...
int raft_is_leader(const RaftState *r) { return r->role == RAFT_LEADER; }
