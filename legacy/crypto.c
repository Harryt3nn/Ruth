...
// Curve25519 keypair 
int crypto_keypair(KeyPair *out) // produces fresh ephemeral keypair, initialises key generation
{
    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_X25519, NULL); // assign keygen context
    if (!ctx) return 0; 
    EVP_PKEY *pkey = NULL; // initialise key generation through OpenSSl, standard X25519 key generation algorithm
...
// OpenSSL stores keys as opaque structures that need to be abstracted:
...
//  ECDH + HKDF to session key; loads keys and zeroises secrets
int crypto_derive_session(const uint8_t *our_priv, const uint8_t *their_pub, SessionKey *out)
{
    // Load our private key (wrapped into OpenSSL key object)
    EVP_PKEY *priv_key = EVP_PKEY_new_raw_private_key(EVP_PKEY_X25519, NULL, our_priv, CURVE25519_KEY_LEN);
    // Load their public key (wrapped into OpenSSL key object)
    EVP_PKEY *pub_key = EVP_PKEY_new_raw_public_key(EVP_PKEY_X25519, NULL, their_pub, CURVE25519_KEY_LEN);
    if (!priv_key || !pub_key) return 0;

    // ECDH (produces 32 byte shared secret)
    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new(priv_key, NULL);
    uint8_t shared[CURVE25519_KEY_LEN];
    size_t  shared_len = sizeof(shared);

    if (EVP_PKEY_derive_init(ctx) <= 0 || EVP_PKEY_derive_set_peer(ctx, pub_key) <= 0 || EVP_PKEY_derive(ctx, shared, &shared_len) <= 0) 
    {
        EVP_PKEY_CTX_free(ctx);
    ...
    // HKDF: shared secret to 32-byte session key
    ...
    if (EVP_PKEY_derive_init(kdf_ctx) <= 0 || EVP_PKEY_CTX_set_hkdf_md(kdf_ctx, EVP_sha256()) <= 0 || EVP_PKEY_CTX_set1_hkdf_salt(kdf_ctx, salt, sizeof(salt)-1) <= 0 || EVP_PKEY_CTX_set1_hkdf_key(kdf_ctx, shared, shared_len) <= 0 || EVP_PKEY_CTX_add1_hkdf_info(kdf_ctx, info, sizeof(info)-1) <= 0 || EVP_PKEY_derive(kdf_ctx, out->key, &key_len) <= 0) 
    { 
    ...
    // Clear shared secret from memory immediately (zeroising)
    memset(shared, 0, sizeof(shared));
    return 1;
}


// haCha20-Poly1305 encrypt 
...
{
    uint8_t nonce[CHACHA_NONCE_LEN]; // random nonce generation, ChaCha20‑Poly1305 requires a unique nonce per key
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_chacha20_poly1305(), NULL, key->key, nonce) <= 0 || EVP_EncryptUpdate(ctx, out_buf + CHACHA_NONCE_LEN, &cipher_len, plaintext, (int)len) <= 0 || EVP_EncryptFinal_ex(ctx, out_buf + CHACHA_NONCE_LEN + cipher_len, &final_len) <= 0) 
    {
...
int crypto_decrypt(const SessionKey *key, const uint8_t *in_buf, size_t in_len, uint8_t *plaintext, size_t *out_len)
{
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_chacha20_poly1305(), NULL, key->key, nonce) <= 0 || EVP_DecryptUpdate(ctx, plaintext, &plain_len, ciphertext, (int)cipher_len) <= 0) 
    { 
        EVP_CIPHER_CTX_free(ctx);
        return 0;
    }
