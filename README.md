...
4. Hashing
5. Encryption
6. Distributed system programming


Entry log 1: 01/04/26 - 23:48 'Init' This is the beginning of the project. In case I actually stick with this and follow it through, I thought it would be nice to journal my thoughts and document the process. Currently, the name of the project has not been determined. The premise is to have a key value database, written in C with end-to-end encryption. When finished, the project can hopefully be exported as an API that I can use in my other projects. 


Entry log 2: 02/04/26 - 17:06 'Sample Code' Phase 2 has been mostly completed, although I don't really understand the codebase that well. Before more code is written, I want to go through and properly understand why things work the way that they do. Currently, the encryption is all server side, and the hash map is still 3rd party. All of this will be changed later. Most of the codebase right now is temporary. 
...
Entry log 8: 10/04/26 - 16:22 'Tokenisation' The final CLI has been coded, that uses a lot of string functions in order to 'tokenise' the user input. The first solution I used, was an array of commands, that included spaces after for commands that required additional values after, such as ruth.set. 'fgets' was used with stdin and strcspn to take the full user input minus \n. Then for basic inputs that are direct command matches, I used strcmp. For the tokenised commands, I used sscanf (which scans strlen rather than stdin). This allowed me to manually set the pointer to after the length of the command. "%255s" was used to seperate words with whitespace and preventing overflow. Right now, a blank function is called as there is no permanent client-server set up. 
